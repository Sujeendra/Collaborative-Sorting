...
static const std::string ARM_RIK_NAME = "/pr2_right_arm_kinematics/get_ik";
static const std::string ARM_LIK_NAME = "/pr2_left_arm_kinematics/get_ik";

typedef actionlib::SimpleActionClient<pr2_controllers_msgs::
                                      JointTrajectoryAction> TrajClient;

class IKTrajectoryExecutor_Right{
...
public:
  IKTrajectoryExecutor_Right(){
    ...
    service = node.advertiseService("execute_cartesian_ik_trajectory_right", 
        &IKTrajectoryExecutor_Right::execute_cartesian_ik_trajectory_right, this);
...
ik_request.ik_request.robot_state.joint_state.name.push_back("r_shoulder_pan_joint");
    ...
    for(int i=0; i<7; i++) ik_request.ik_request.robot_state.joint_state.position[i] = start_angles[i];
  ...
  bool execute_cartesian_ik_trajectory_right(
...
class IKTrajectoryExecutor_Left{
...
public:
  IKTrajectoryExecutor_Left(){
    ...
    //wait for the various services to be ready
    ROS_INFO("Waiting for services to be ready");
    ros::service::waitForService(ARM_LIK_NAME);
    ROS_INFO("Services ready");
    ...
    //register a service to input desired Cartesian trajectories
    service = node.advertiseService("execute_cartesian_ik_trajectory_left", 
        &IKTrajectoryExecutor_Left::execute_cartesian_ik_trajectory_left, this);
...
ik_request.ik_request.robot_state.joint_state.name.push_back("l_shoulder_pan_joint");
  ...
  //service function for execute_cartesian_ik_trajectory
  bool execute_cartesian_ik_trajectory_left(
         ik_trajectory_tutorial::ExecuteCartesianIKTrajectory::Request &req,
         ik_trajectory_tutorial::ExecuteCartesianIKTrajectory::Response &res){
...
int main(int argc, char** argv){
  ...
  IKTrajectoryExecutor_Right ik_traj_exec_r = IKTrajectoryExecutor_Right();
  IKTrajectoryExecutor_Left ik_traj_exec_l = IKTrajectoryExecutor_Left();
